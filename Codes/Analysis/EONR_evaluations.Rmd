...
cc_consisten_b105 <- readRDS(here::here("Data", "Processed-Data", "corn_corn_consistent_B105.rds"))
...
cc_consistant_td <- data.frame(
  applied_N = as.numeric(applied_N$year_4[1:100]),
...
ggplot()+
  geom_point(aes(x = applied_N, y = yield_max, group = plot_id), data = cc_consisten_b105_yield, size = 0.5)+
  geom_line(aes(x = applied_N, y = yield_max, group = plot_id), data = cc_consisten_b105_yield, size = 0.5)+
...
```{r cache=TRUE}
###---- upload the simulation result ----###
cc_ofpe_b105 <- readRDS(here::here("Data", "Processed-Data", "corn_corn_ofpe_B105.rds"))
...
cc_ofpe_td <- data.frame(
  applied_N = round(as.numeric(applied_N$year_4[1:100]), digit = 2),
  plot_id = as.character(seq(1,100, by = 1))
) %>% 
